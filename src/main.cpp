...
NVSLoader nvsLoader(NVS_NAMESPACE);
const char KEY_DEVICE_KEY[] = "devicekey";
const char KEY_DEVICE_URL[] = "deviceurl";
char devicekey[64 + 1] = {0};
char deviceurl[128 + 1] = {0};

const char KEY_BLUB_IP[] = "blubip";
char ipaddrstring[16 + 1] = {0};
IPAddress ipaddr;


WiFiController wifiController(NVS_NAMESPACE);
HTTPClient httpCelient;
...
void ESP_LOGI1(const char* tag, const char* msg){
    M5.Display.println(msg);
}

void ESP_LOGI2(const char* tag, const char* msg, const char* s){
    M5.Display.printf(msg, s);
    M5.Display.println();
}

void ESP_LOGE2(const char* tag, const char* msg, const char* s){
    M5.Display.printf("error:");
    M5.Display.printf(msg, s);
    M5.Display.println();
}
BulbClient* GetClient(int clienttype, WiFiClient &wificlient, HTTPClient& http, const char* key, const char* url, IPAddress ipaddr) {
    BulbClient *client;
    if (clienttype == 0) {
        client = new MerossClient(wificlient, http, key , url);
    } 
    if (clienttype == 1) {
        client = new WizClient(ipaddr);
    } 
    return client;
}
...
static void hid_print_new_device_report_header(hid_protocol_t proto) {
  ...
  if (prev_proto_output != proto) {
    ...
    if (proto == HID_PROTOCOL_MOUSE) {
      //M5.Display.printf("Mouse\r\n");
    } else if (proto == HID_PROTOCOL_KEYBOARD) {
      //M5.Display.printf("Keyboard\r\n");
    } else {
      //M5.Display.printf("Generic\r\n");
    }
    fflush(stdout);
  }
}
...
static inline void hid_keyboard_print_char(unsigned int key_char) {
    ...
    if (key_char == 0x31
     ...
     || key_char == 0x36
     || key_char == 0x37) {
      uint8_t ctrlId = key_char;
      M5.Display.printf("0x%02X ", key_char);
      xQueueSend(xQueue_1, &ctrlId, portMAX_DELAY);
     }
  ...
  if (!!key_char) {
    ...
    if (KEYBOARD_ENTER_MAIN_CHAR == key_char) {
      ...
      M5.Display.printf("keyboard:%c", key_char);
    }
...
static void hid_host_mouse_report_callback(const uint8_t *const data,
  ...
  M5.Display.printf("X: %06d\tY: %06d\t|%c|%c|\r", x_pos, y_pos,
 ...
 * @param[in] length  Length of input report data buffer
 */
void hid_host_generic_report_callback(const uint8_t *const data,
                                             const int length) {
  ...
  if (length > 3) {
    ...
    if (data[3] == 0x62
     ...
     || data[3] == 0x5C
     || data[3] == 0x5D) {
      uint8_t ctrlId = data[3];
      M5.Display.printf("0x%02X ", data[3]);
      xQueueSend(xQueue_1, &ctrlId, portMAX_DELAY);
    } else {
      M5.Display.printf("1x%02X ", data[3]);
      return;
    }
  } else {
    M5.Display.println("bad report");
    return;
  }
  putchar('\r');
  putchar('\n');
...
bool GetMutex() {
  BaseType_t xStatus = xSemaphoreTake(xMutex, 100);
  return (xStatus == pdTRUE);
}
bool AutoTemperatureMode = true;
bool ColorMode = false;
void meross_client_control_thread(void *args) {//スレッド ②
  bool ret = false;
  ...
  while (1) {
    ...
    BaseType_t recv = xQueueReceive(xQueue_1, &ctrlId, 0);
    if (recv == pdTRUE) {
      //M5.Display.printf(" p ");
      if (ctrlId == 0x62 || ctrlId == 0x31) {
        if (GetMutex()) {
          BulbClient* client = GetClient(1,wifiController.getClient(), httpCelient, devicekey , deviceurl, ipaddr);
          if (onoff_status == 0) {
            ret = client->TurnOn();
          }
          if (onoff_status == 1) {
          ...
          if (ret) {
            onoff_status = onoff_status == 0 ? 1 : 0;
          }
          delete client;
		      xSemaphoreGive(xMutex);
        }
      ...
      //btn2
      else if (ctrlId == 0x59 || ctrlId == 0x32) {
        ColorMode = false;
        if (GetMutex()) {
          // 色温度モード
          BulbClient* client = GetClient(1,wifiController.getClient(), httpCelient, devicekey , deviceurl, ipaddr);
          ret = client->SetTemperature(0);
          if (ret) {
            onoff_status = 1;
          }
          delete client;
      ...
      //btn3
      else if (ctrlId == 0x5A || ctrlId == 0x33) {
        ColorMode = true;
        if (GetMutex()) {
          BulbClient* client = GetClient(1,wifiController.getClient(), httpCelient, devicekey , deviceurl, ipaddr);
          ret = client->SetRgb(255, 0, 0);
          if (ret) {
            onoff_status = 1;
          }
          delete client;
      ...
      //ダイアル押す
      else if (ctrlId == 0x5B || ctrlId == 0x37) {
        AutoTemperatureMode = AutoTemperatureMode ? false : true;
      }
      // 左右
      else if (ctrlId == 0x5C || ctrlId == 0x5D  || ctrlId == 0x35 || ctrlId == 0x36) {
        if (vec.empty()) {
          delay(50);
        }
        vec.push_back(ctrlId);
        continue;
      }
    } else {
      if (!vec.empty()) {
        ...
        for (const auto& e : vec) {
           level += (e == 0x5c || e == 0x35) ? -2 : 2;
        }
        vec.clear();
        if (GetMutex()) {
          BulbClient* client = GetClient(1,wifiController.getClient(), httpCelient, devicekey , deviceurl, ipaddr);
          ret = client->SetRelativeLuminance(level);
          if (ret) {
            onoff_status = 1;
          }
          delete client;
...
void adaptive_lite_control_thread(void *args) {//スレッド ②
  xSemaphoreGive(xMutex);
  BaseType_t xStatus;
  while (1) {
    //100tick == 1000ms
    time_t now = time(NULL);
    if (now < 1708241269 || !AutoTemperatureMode || ColorMode) {
      delay(1 * 60 * 1000);
      continue;
    }
    xStatus = xSemaphoreTake(xMutex, 100);
    if (xStatus != pdTRUE) {
      continue;
    }
    // 電源off
    BulbClient* client = GetClient(1,wifiController.getClient(), httpCelient, devicekey , deviceurl, ipaddr);
    int onoff = client->GetState();
    if (onoff != 0 && onoff != 1) {
      delete client;
      xSemaphoreGive(xMutex);
      delay(1 * 60 * 1000);
    ...
    SunPosition sun(36.61211, 138.183, now, 9);
    double radian = sun.altitude() * PI / 180.0;
	  double altitude = radian * 2.0 / PI;
    ...
    bool ret = client.SetTemperature(temperaturelevel);
    */
    bool ret = client->SetTemperatureByAltitude(altitude);
    delete client;
	  xSemaphoreGive(xMutex);
    if (ret){
      M5.Display.println(" ok");
...
void hid_host_interface_callback(hid_host_device_handle_t hid_device_handle,
  ...
  ESP_LOGI(TAG, "interface call back");
  ...
  case HID_HOST_INTERFACE_EVENT_DISCONNECTED:
    ESP_LOGI2(TAG, "HID Device, protocol '%s' DISCONNECTED",
             hid_proto_name_str[dev_params.proto]);
    ESP_ERROR_CHECK(hid_host_device_close(hid_device_handle));
    break;
  case HID_HOST_INTERFACE_EVENT_TRANSFER_ERROR:
    ESP_LOGI2(TAG, "HID Device, protocol '%s' TRANSFER_ERROR",
             hid_proto_name_str[dev_params.proto]);
    break;
  default:
    ESP_LOGE2(TAG, "HID Device, protocol '%s' Unhandled event",
             hid_proto_name_str[dev_params.proto]);
    break;
  }
}
...
static void usb_lib_task(void *arg) {
  ...
  while (gpio_get_level(APP_QUIT_PIN) != 0) {
    ...
    if (event_flags & USB_HOST_LIB_EVENT_FLAGS_NO_CLIENTS) {
      ...
      ESP_LOGI1(TAG, "USB Event flags: NO_CLIENTS");
    }
    // All devices were removed
    if (event_flags & USB_HOST_LIB_EVENT_FLAGS_ALL_FREE) {
      ESP_LOGI1(TAG, "USB Event flags: ALL_FREE");
    }
  }
  // App Button was pressed, trigger the flag
  user_shutdown = true;
  ESP_LOGI1(TAG, "USB shutdown");
  // Clean up USB Host
...
void app_main(void) {
  ...
  //ESP_LOGI1(TAG, "HID Host example");
  ...
  task_created =
      xTaskCreatePinnedToCore(usb_lib_task, "usb_events", 4096,
                              xTaskGetCurrentTaskHandle(), 3, NULL, 1);
   ...
   */
  task_created =
      xTaskCreatePinnedToCore(&hid_host_task, "hid_task", 4 * 1024, NULL, 3, NULL, 1);
  assert(task_created == pdTRUE);
}
...
void setup() {
  ...
  M5.Display.clear();
  M5.Display.setTextSize(1);
  nvsLoader.begin(NVS_CONFIG_PATH);
  nvsLoader.get(KEY_DEVICE_KEY, devicekey, sizeof(devicekey));
  nvsLoader.get(KEY_DEVICE_URL, deviceurl, sizeof(deviceurl));
  nvsLoader.get(KEY_BLUB_IP, ipaddrstring, sizeof(ipaddrstring));
  ipaddr.fromString(ipaddrstring);
  xMutex = xSemaphoreCreateMutex();
  xQueue_1 = xQueueCreate(100, sizeof(uint8_t));
  //xQueueCreate( [待ち行列の席数], [一席あたりのバイト数] );

  //スレッドの準備
  xTaskCreatePinnedToCore(meross_client_control_thread, "eross_client_control", 4096, NULL, 2, &thp[0], 0);
  xTaskCreatePinnedToCore(adaptive_lite_control_thread, "adaptive_lite_control", 4096, NULL, 1, &thp[1], 0);
  ...
  if (wifiController.begin(NVS_CONFIG_PATH, connectingWiFiCallback)) {
      struct tm localTime;
